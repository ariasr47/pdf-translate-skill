...
September. A02 and A06 were then built as separate changes from main, on the
local branches `fix/a02-page-count` and `fix/a06-rebuild-mapping-checks`; both
await integration.

The [smallest-items shortlist](../../docs/PUBLIC-READINESS-QUICK-WINS-2026-09-19.md)
is a dated reading aid, not a second status tracker. **A04, A06 and A15 have
completed small documentation slices; their broader items remain open. A01 and
A03 are done.** Rows not marked otherwise remain unstarted. Sizes and proposed
...
| **A06 · P1** | Existing 01/09 verification behavior; **built on local branch `fix/a06-rebuild-mapping-checks` with the examples slice (23 September); a completed full-suite run and the independent pass are still owed; awaiting integration**. Audit R6; [example evidence](../../docs/reviews/2026-09-20-verification-examples.md). | **Supply mapping context in rebuild and examples. S; examples-only slice XS.** Include final-artifact verification and a target-script fill sample. | The two-label/one-empty-target fixture fails through the documented default rebuild route, with `empty-targets` reported. Verify the audit probe and quickstart commands; explicit caller options remain honored. Docs-only repair does not close the default-command defect. |
...
**Next, as of 23 September:** A02, then A06, each a separate change from main.
That is the order the 22 September review and the app task agreed. After them,
the smallest items are A21's explicit validation of both manifests and its
separate type-check environment slice. A04's wording, A06's verification
examples and A15's README facts are complete as slices; the broader items remain
open. A01 and A03 are done. These next steps remain recommendations. A07/A08
