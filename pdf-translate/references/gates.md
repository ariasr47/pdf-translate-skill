...
`--min-ink`, `--report verify_report.json`, `--fail-on-review`, `--reference-fonts DIR` (the Noto Sans JP / SC variable faces gate 20 compares against; default `tests/fonts/` of a checkout — an installed package has none, so a service passes its own). Omit
