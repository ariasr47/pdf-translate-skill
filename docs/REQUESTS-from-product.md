...
## Current coordination status — 26 September 2026, product advice on what is left

**Main is v81 at `0d58202`** (records at `aae125c`). Nothing is in flight.

**Rodrigo asked the product,** do or wait, on each open item. Its advice
(26 September, which Rodrigo decides) is recorded on each row. In short:
- **Why the order:** the app adopts v81 as one change, right after its
  engine-boundary build lands, which is days away. After that, its
  library route for Japanese targets uses extract, strip and legacy
  retypeset, so legacy-route defects come first.
- **Do now, in this order:**
  1. a consumer upgrade note from v54 to v81, plus a `v81.0.0` tag (new
     row);
  2. a docs batch: capture docs with a `capture_dir=""` test, the
     `document` block, and #42's rewording;
  3. right-to-left runs placed one source-width right;
  4. a non-+x multi-span line's box;
  5. rotated runs: refuse horizontal-only features by name;
  6. the CLI pair, `--flag=value` and `python -m` logging;
  7. typography verify's widths and unrotated page, together;
  8. one time-boxed attempt to reproduce the ink-ratio observation.
- **Wait:** #42's real fix (until typography-1 is next worked on);
  `field_fonts`'s `/FT` inheritance (until a real form needs it); the full
  reading-frame layout for rotated runs (until the app's adoption
  comparison).
- **Drop for now:** the source's whole `/Lang` in `document.lang`.
- **Housekeeping:**
  - The 11 merged branches: the product has no reason to keep them. The
    app's v54 and v81 pins are both reachable from `origin/main`.
  - The two local backup refs: everything in them is on `main`. Three
    commits landed as identical patches, and R-03's is the pre-stack
    version of `dc87cce`, differing only by two DECISIONS.md lines that
    `main` has.
  - Deletion is Rodrigo's to approve.

**Unchanged:** E12 is blocked on its six rulings, and R-94 is "not yet".
The app stays on v54 until the adoption.

## Coordination status — 25 September 2026, stack merged (historical snapshot)

**Main is v81 at `0d58202`.** Nothing is in flight. Rodrigo told this
session to proceed with its recommendations, and it merged #42 to #52 in
order, each as a merge commit:
- Before each merge, the ubuntu and windows py3.14 jobs and the plugin
  manifest job had passed on the exact head.
- Each merge was pinned to that head (`--match-head-commit`).
- A stacked PR's base was moved to `main` just before its merge.

| PR | contents | version | merge commit |
|---|---|---|---|
| #42 | R-42, a typography content refusal names its text | 72 | `b0ed899` |
| #43 | R-110, SPDX licence expression | 73 | `bd30193` |
| #44 | R-100, dead code removed | 74 | `019405b` |
| #45 | R-99, ResourceWarnings 45 → 0 | 75 | `b7cac4c` |
| #46 | R-79, R-80, R-82, R-83, the shipped docs match the code | 76 | `bb234e0` |
| #47 | R-51, CJK test instances built once (tests only) | 76 | `5e8a82a` |
| #48 | R-48 with R-49, exact pixel counts, oracle scoped to its pages | 77 | `eb996ca` |
| #49 | R-50, `prepare_font` instances the job's subset; canary run 4 | 78 | `0a0914e` |
| #50 | extract keeps an authored `widget_text.json` | 79 | `8196418` |
| #51 | `/Lang` keeps the whole tag, and verify reads it whole | 80 | `332d9c6` |
| #52 | verify gate 22, `widget-text` | 81 | `0d58202` |

**Checked before and after:**
- **Before merging,** all 11 merges were rehearsed in a scratch worktree.
  They merged cleanly. `7c91b4a`, pushed to #46 after #47 branched, joined
  the later work's `references/retypeset.md` without conflict.
- **The rehearsed tree passed CI's commands on macOS:** 789 tests, 0
  ResourceWarnings, and only the known macOS-only `HotLoopTests` failure
  plus the one expected failure. Canary, repository tests and eval
  fixtures exit 0.
- **`main`'s tree is byte-identical to the rehearsed one.**
- **CI on `main`** at `0d58202` passed ubuntu and windows py3.14 and the
  plugin manifest.
...
- extract recording the source's whole `/Lang`, which the product rates
  low.
...
## Coordination status — 25 September 2026, after close (historical snapshot)

**Main is v71 at `62e87bf`.** Eleven things are in flight, each stacked on
the one before:
- **#42** to **#50,** as below;
- **#51,** the `/Lang` fix at v80. The product checked all 13 of its tags:
  identical;
- **#52,** verify gate 22 at v81. Rodrigo approved the push in this
  session.

**For a consumer, v81:** verify gains gate 22, `widget-text`, which is
REVIEW-only. `run_verify` and `verify` take `widget_text=` (a path or a
dict), and the CLI takes `--widget-text`. A consumer that passes
`fail_on_review` exits 1 on any unchanged widget text it has not marked
as kept. The app runs no library verify.
...
## Coordination status — 25 September 2026, close of day (historical snapshot)

**Main is v71 at `62e87bf`.** Ten things are in flight, each stacked on
the one before:
- **#42** to **#48,** as below;
- **#49,** R-50 at v78, with canary run 4's records and its translation
  comparison;
- **#50,** the `widget_text` overwrite fix at v79. The product checked it:
  identical on its call shape apart from the new result key;
- **the `/Lang` fix at v80,** on `fix/lang-keeps-region`. It waits for
  Rodrigo's go to push.

**For a consumer, v80:**
- `/Lang` is the mapping's whole tag, `es-US` and not `es`, and the
  console line names what the file holds.
- Verify reads `/Lang` whole. A typography build for a region tag no
  longer FAILs its metadata gate.
- For a region-less tag the file is byte-identical to v79's.

**Next:** filed rows wait for Rodrigo's order. The product advises the
verify gate for widget text left in the source language next, at medium
priority. The others:
- the `document` block;
- #19's capture docs;
- #42's line-start position.
...
## Coordination status — 25 September 2026, end of day (historical snapshot)
...
- **#47,** R-51, tests only;
- **#48,** R-48 with R-49 at v77;
- **R-50 at v78,** on `perf/r50-subset-before-instancing`, with canary
  run 4's records. It waits for Rodrigo's go to push.

**For a consumer, v78:**
- `prepare_font` leaves no `<out>.instanced.ttf` or `<out>.chars.txt`. A
  field face is the variable face itself, which `field_fonts` instances.
- The prepared subset draws the same but is not byte-identical: some
  whole-face summary fields now describe the job's glyphs, and GPOS is
  more compact.
- The app calls neither `prepare_font` nor `field_fonts`.

**Canary run 4** put Opus 5.5 beside Opus 5, on the same v77 skill: 5/5
each, at the same cost. Three proposals came out of it, filed below.

**Next:** the performance items are done. The review's assigned list is
complete, and E12 waits on its rulings. Filed rows wait for Rodrigo's
order:
- #19's capture docs;
- #42's line-start position;
...
The product advises the overwrite first.
...
- The product checked v77 on its call shape and found it identical; no
  asks.
...
| 2026-09-24 | **Proposal from this library: lay out rotated runs in their own reading frame, or refuse horizontal-only features on them by name.** A run whose line is not horizontal in PDF space silently loses dot leaders and tails, `right`, `center` and override `x`; its merges squeeze into a column, and inline markup is drawn as literal tags. | Found while building item 3. Landscape pages, whose content is counter-rotated to read upright under `/Rotate`, send every run down this path. On Rule 1's 12-run landscape fixture, v64 puts every run at its source origin (v63 put none there), but `right`, `center` and leader filling do nothing. | **not assigned** — the product filed it as a proposal on 24 September. Rotated pages are 0 of 449 in the wild set and 0 of 1,199 in the app's sample, so it waits until landscape forms or vertical Japanese matter. Rodrigo can raise it. **Product advice** (26 September, for Rodrigo, who decides): fifth, the minimal version now: refuse horizontal-only features on rotated runs by name, loud rather than silent, since inline markup drawn literally on a rotated line would put a visible `<b>` in a delivery. The full reading-frame layout waits for the app's v81 adoption comparison on FL-150, FL-142 and its staging fixtures, which shows whether rotated runs occur in its documents. |

| 2026-09-24 | **Proposal from this library: a right-to-left source run is rebuilt where the source drew it.** MuPDF reports a right-to-left run's origin at its right end, and retypeset draws every run rightward from its origin. So each run of an Arabic or Hebrew source lands one source width to the right: `corpus/ar_source.pdf`'s title spans x 60.9–147.6 and its identity rebuild 140.6–239.9. No verify gate reads positions. | Found by R-64's position check on 24 September. It affects documents whose SOURCE is right-to-left, not right-to-left targets. The wild set has none. Pinned as an expected failure in `tests/test_corpus_verdicts.py`, so a fix shows up. | **proposed** 24 September. **not assigned** — the product answered the same day that it keeps right-to-left out of its MVP. The expected-failure pin holds the defect until Rodrigo decides on right-to-left for the product. **Product advice** (26 September, for Rodrigo, who decides): do now, third. After v81 adoption it is on the app's route: an Arabic or Hebrew source into a Japanese target. The misplacement is visible, and no gate catches it. |

| 2026-09-24 | **Proposal from this library: a multi-span line keeps its full box when its direction is not +x.** `extract_segments.py`'s span union keeps the first span's left edge, so a line whose spans advance another way loses the rest. Before item 3, a two-span line on a `/Rotate 180` page measured 50 pt wide instead of 161. | Found in item 3's coordinate map. Since item 3, it affects only lines that are not horizontal in PDF space. | **proposed** 24 September; the app noted it. **Product advice** (26 September, for Rodrigo, who decides): do now, fourth. It is on the app's route after adoption, small and concrete, and 50 pt instead of 161 corrupts the width budget. |

| 2026-09-24 | **Proposal from this library: typography-1 measures against the unrotated page.** `typography_verify.py` tests unrotated glyph boxes against the rotated `page.rect`, so a correct typography-1 run near the edge of a `/Rotate 90` page would FAIL. Retypeset's typography placement budget (`_measure_typography`) also reads `page.rect`: called directly at `/Rotate 90`, it refuses a run that fits at `/Rotate 0` (Rule 1, item 3). | Found in item 3's coordinate map. It cannot happen today: typography-1 retypeset refuses rotated pages, and group E is held. | **proposed** 24 September; the app noted it. **Product advice** (26 September, for Rodrigo, who decides): do now, seventh, together with the widths row: the same area, on rotated pages. |

| 2026-09-24 | **Proposal from this library: `python -m pdf_translate.retypeset` and `python -m pdf_translate.verify` print their log.** Run that way, both print nothing: `runpy` re-executes the already imported module as `__main__`, whose logger is not under `pdf_translate`. The exit code and the output file are right. | Found by item 3's Rule 1 review. `scripts/*.py` and `pipeline.py` print normally, and the same happens on v63. | **proposed** 24 September. **Product advice** (26 September, for Rodrigo, who decides): do now, sixth, as a "CLI correctness" pair with the `--flag=value` row. The app calls functions, never the CLIs, so there is no app impact. A plugin user who runs `-m` gets an exit code of 1 with the refusal lines hidden, and cannot tell what to shorten or which glyph is missing. A test runs both modules with `-m` and asserts that their log reaches stdout. |

| 2026-09-24 | **Proposal from this library: verify refuses an input flag spelled `--flag=value` instead of ignoring it.** Verify reads its options only as `--flag value`. A caller who writes `--source-words-from=segments.json`, `--translations=…` or `--reference-fonts=…` gets a run with that input silently absent: the gates that need it SKIP or run without it. | Found by R-05's review. `pipeline.py rebuild` now protects a file named in either spelling from being overwritten by OUT, but verify itself still ignores the `=` form. | **proposed** 24 September. **held** the same day with review group C, under Rodrigo's ruling A. The app runs no library verify: it confirmed that `apps/api/api/` makes no verify or `qa_check` call and passes none of these flags. Revisit if the app adopts verify. **Product advice** (26 September, for Rodrigo, who decides): do now, sixth. Off the app's route, which runs no library verify, but it silently drops a plugin user's input, and it is small. |

| 2026-09-25 | **Proposal from this library: typography verify accepts the widths MuPDF actually embeds.** MuPDF writes each glyph's width to the PDF's `/W` array in whole thousandths of an em, rounded down. Typography verify predicts each glyph's position from the font program's exact advance. In a face whose units-per-em is not 1,000, every glyph is drawn a little left of the prediction, and the gap passes the 0.05 pt tolerance within about 10–15 glyphs. Verify then FAILs a correct line with "Run N is not at its ordered, uniformly scaled position". Arial, Times New Roman and most TrueType faces use 2,048. | Found while measuring R-47. On an identity typography-1 build in Arial, verify FAILed all 150 of 150 correct lines. In Noto Sans, which uses 1,000 units, the same job PASSes. For Arial Regular's P, 1366/2048 em is 666.99 thousandths and is embedded as 666. All 20 embedded widths equal the rounded-down value, and 9 of them differ from normal rounding. The typography-1 tests and probes use only 1,000-unit Noto faces. | **proposed** 25 September. Typography-1 is group E, which is held, and the app has not adopted it. A fix would be one of: (a) verify predicts positions from the embedded `/W` widths, (b) retypeset embeds exact widths, if MuPDF can be made to, or (c) the tolerance scales with the run's length. Each needs Rule 1. **Product advice** (26 September, for Rodrigo, who decides): do now, seventh, together with the unrotated-page row. A false FAIL on correct output with Arial, Times and most 2,048-unit faces makes typography verify unusable for plugin users. Off the app's route, which does not use typography-1, so it follows the legacy-route items. |

| 2026-09-25 | **Proposal from this library: `field_fonts` finds a field's type however far up it is inherited.** It decides whether a widget is a text or choice field from the widget's `/FT` or its parent's. A field whose `/FT` is set only on a grandparent or higher is skipped: its `/DA` is never pointed at the embedded font, so typed target-script text can still fall back or vanish. | Found by R-33's Rule 1 pass, and reproduced here: a text field typed only on its grandparent gets 0 fields rewritten and stays on `/Helv`, on v69 and v70 alike. None of the 17 wild PDFs is built that way. The fix would walk the whole `/Parent` chain for `/FT`, as `inherited_da` already does for `/DA`. | **proposed** 25 September. **Product advice** (26 September, for Rodrigo, who decides): wait until a real form needs it. It is absent from the wild corpus, and the app does not call `field_fonts`. |

| 2026-09-25 | **Request from the product: capture's off switch and contents are documented.** Three facts. `capture_dir=""` is an explicit off switch that wins over `PDF_TRANSLATE_CAPTURE_DIR`. A bundle holds every source string and the form's fields even without `original=`. With `PDF_TRANSLATE_CAPTURE_BELOW` set, a successful build writes a bundle too. **Acceptance:** each fact is stated in `references/consumer-guide.md`, and `""` beating the environment variable is locked by a test. | From the product's review of #19 (merged). Today `""` works as off only because it is falsy: `resolve_capture_dir("")` returns `""`, which beats the variable, and callers test it with `if`. Its docstring says the return is `None` either way, which is not true for `""`. The app will pass `""` and never set either variable in production. | **requested** 25 September, by the app's session in its PR review. Rodrigo decides when. **Product advice** (26 September, for Rodrigo, who decides): do now, in a docs batch with the `document` block and #42's rewording, second after the upgrade note. The app will pass `capture_dir=""` at adoption, so a test locks "the empty string wins over the environment variable". The docs also say what a bundle holds without `original=`, and that `CAPTURE_BELOW` captures successful builds too. |

| 2026-09-25 | **From the product's review: a typography content refusal names the occurrence where its offending text starts, even on a split line.** R-42 records the line matrix's start, not the start of the offending text. So when a line holds more than one occurrence, and the offending text follows other text, the refusal names the wrong one. **Acceptance:** a test where the offending text follows other text on the same line names the right occurrence. The fix locates the offending show's first glyph from the MuPDF text trace, as the occluded-text branch already does. Alternatively, the `ContentIssue.at` comment, the R-42 evidence and its DECISIONS row say "start of the text line". | Reported by the product's review of #42, on the typography-1 path, which the app never reaches. It has not been reproduced here yet. It is low priority. | **proposed** 25 September. Rodrigo decides whether to fix it or reword it, and when. **Product advice** (26 September, for Rodrigo, who decides): reword now, in the docs batch: the `ContentIssue.at` comment and the evidence say "start of the text line". The real fix, locating the offending glyph, waits until typography-1 is next worked on. |

| 2026-09-25 | **Proposal from this library: `pipeline.py init --widget-text` never overwrites the file it reads.** SKILL.md says to run `init` bare for the `widget_text.json` scaffold, author the targets, then run it again with `--widget-text`. The second run applies the targets, then extract writes a fresh scaffold over `WORK/widget_text.json`, the file the author just filled in. **Acceptance:** after `init --widget-text WORK/widget_text.json`, the authored targets are still in that file, and the stripped PDF carries them; a test on `corpus/choice_fields.pdf`. | Found by canary run 4: Opus 5.5 reported it, and Opus 5 avoided it by authoring into a separate file. Reproduced here: on `corpus/choice_fields.pdf`, 5 authored targets became 5 nulls. An author who re-runs from the empty file ships untranslated tooltips; the model avoided it by keeping a copy. | **proposed** 25 September. Product advice (25 September, for Rodrigo to schedule): first, after R-50. It is silent data loss in the workflow SKILL.md prescribes. No effect on the app, which passes its mapping to `strip_text` in memory and never runs `init`. **Assigned** 25 September: Rodrigo said yes in this session, taking the product's advice to do it first after R-50. **Correction to this row's reason:** re-applying the emptied file is refused ("target is null"), so the work is lost loudly, not shipped. The silent path is a bare re-run of `init`, which ships source-language widget text that verify has no gate for (proposal below). **Built** 25 September, v79, branch `fix/widget-text-init-keeps-authored`, stacked on #49. Extract never writes a fresh scaffold over a `widget_text.json` that holds authored text: a non-null target, a string shorthand, or a file that is not JSON. It keeps the file, says so, and sets `ExtractResult.widget_text_kept`. A file whose every target is null is still refreshed. On `corpus/choice_fields.pdf`, `init --widget-text WORK/widget_text.json` now leaves the file byte-identical and the stripped PDF carries all 5 targets. `tests/test_widget_text_authored.py` has 6 tests, all failing on `ef03a75`. The app's session checked its code read-only: it never writes a `widget_text.json`, reads the extract result only by `cores`, `segments` and `warnings`, and its call shape stays byte for byte as before. Evidence: `docs/reviews/2026-09-25-widget-text-kept.md`. Rodrigo approved in this session: it is PR #50, stacked on #49. **The product checked v79** (`c148b73`), in advice for Rodrigo: no reason to hold it. It read `f0a171c`, and ran its app-shaped harness on v77 (`f273380`) against v79. All 16 cases were identical apart from extract's new `widget_text_kept` key; 89 of 89 files were byte-identical with the trailer `/ID` masked, every `widget_text.json` scaffold among them; both 40-run cases matched. **done** — v79, PR #50 (merge `8196418`, 25 September). |

| 2026-09-25 | **Proposal from this library: `/Lang` keeps the whole language tag.** Retypeset sets `/Lang` with PyMuPDF's `set_language`, which drops a region subtag: `es-US` becomes `es` and `pt-BR` becomes `pt`, while `zh-Hans` is kept. Retypeset's log still says `/Lang -> es-US`. **Acceptance:** a mapping `lang` of `es-US` gives `/Lang (es-US)` in the output, and the log names what was written. | Found by canary run 4; both models noted it. Reproduced here with PyMuPDF 1.28.2. Verify's metadata gate compares the primary subtag, so it passes today. Screen readers and hyphenation read the region. | **proposed** 25 September. Product advice (25 September, for Rodrigo to schedule): a quick win. At least make the log truthful; better, write the full tag, with verify's comparison unchanged. No effect on the app, whose 16 tags carry no region subtag. **Assigned** 25 September: Rodrigo said yes in this session. **Built** 25 September, v80, branch `fix/lang-keeps-region`, stacked on #50. Retypeset writes the catalog's `/Lang` itself, as the mapping spells it, and its console line names what the file holds. Reproduced first: `set_language` kept `zh-Hans` and `ja` whole but cut `es-US`, `pt-BR`, `es-419`, `en-GB`, `zh-Hant-TW`, `sr-Latn-RS` and `de-CH-1996` to their first subtag, and wrote `Japanese` as `jap`. **Two readers were wrong too:** PyMuPDF's `doc.language` cuts a whole stored tag the same way. So a typography build for `en-US` already FAILed verify's metadata gate ("expected en-US; output declares en"), and with no mapping `lang`, han-forms read a `zh-Hant-TW` output as `zh`, Simplified. Verify now reads `/Lang` as stored. The legacy comparison is unchanged, so an older output with `es` still passes an `es-US` mapping. The app's call shape: for 20 region-less tags, the old and new writes give byte-identical files. Extract still records the source's `/Lang` through the shortening getter; that is on the app's route and outside this request, so it is left alone and raised with the app. `tests/test_lang_tag.py` has 6 tests, all failing on `c148b73`. Evidence: `docs/reviews/2026-09-25-lang-keeps-region.md`. **The product checked v80** (`cc9595e`), in advice for Rodrigo, and corrected this row's count: the app has 13 tags, not 16. They are es, pt, fr, de, it, pl, ro, tr, vi, uk, ru, zh-Hans and ja, none with a region. Its probe ran on v79 (`c148b73`) and v80, each in its own Python 3.14 venv, on `corpus/choice_fields.pdf`. It extracted, stripped, and ran a loud retypeset with `lang` set to each tag. All 13 were identical: return codes, stdout, the stored `/Lang` string, and output bytes with the trailer `/ID` masked. That includes `ro`, which this repo's 20-tag check did not cover. The app never reads segments.json's `document` block. **done** — v80, PR #51 (merge `332d9c6`, 25 September). |

| 2026-09-25 | **Proposal from this library: document the mapping's `document` block.** `review.py` fills the reviewer prompt's identity slots from a `document` block in `translations.json` (`class`, `issuer`, `parallel_text`, …), at line 665. `references/translations-format.md` does not mention it, so an author cannot know to write it. **Acceptance:** the format reference documents the block and its keys, and SKILL.md's identity step says where the record goes for the review prompt. | Found by canary run 4 and confirmed in the code. The run filled the block by reading `review.py`. | **proposed** 25 September. Product advice (25 September, for Rodrigo to schedule): docs only, with a docs guard like R-79's; no effect on the app, which never runs `review.py`. Canary run 4 also saw page 1's ink ratio read 1.18 on `out.pdf` and 1.32 on `final.pdf`, for pages the model rendered pixel-identical. It is recorded, not reproduced or diagnosed here. Product advice: reproduce it before any work. Two pixel-identical renders should not give different ink ratios, so this bears on verify's trustworthiness. **Product advice** (26 September, for Rodrigo, who decides): do now, in the same docs batch, with a docs guard like R-79's. On the ink-ratio observation: one reproduction attempt, time-boxed to about an hour, eighth in the order. If it reproduces, it is a verify-trust defect, filed with its cause; if not, it is closed as not reproduced, with what was tried. |

| 2026-09-25 | **Proposal from this library: verify flags widget text left in the source language.** Verify's leak scan reads page text, and its `/Opt` gate checks only that export values did not move. Nothing checks tooltips (`/TU`), dropdown display labels or text-field defaults. A bare `init`, or a strip without `--widget-text`, ships them in the source language, and no gate reads them: `verify.py` never reads `/TU`, and reads `/Opt` only for export parity (checked in the code, not by a verify run). **Acceptance:** on `corpus/choice_fields.pdf` stripped without `--widget-text`, verify reports the untranslated dropdown labels by field. With them translated, or deleted from the mapping on purpose, it does not. Verify by a test on that file. | Found while building the `widget_text` overwrite fix: after a bare re-run, stripped.pdf carried the English dropdown labels. The app's session suggested filing it. A label that is data (a code, an ID) stays in the source language on purpose, so the verdict is likely REVIEW, not FAIL. | **proposed** 25 September. The app's session suggested it. **Product advice** (25 September, for Rodrigo to schedule): medium priority. Take it after v79 lands, before the `/Lang` and `document`-block quick wins. After v79 the remaining silent case is a plugin user who never authors widget text, whose visible labels and tooltips ship in the source language; `/Lang` is only metadata. REVIEW, not FAIL. No app need: the app does not run library verify, and its own route reports widget text left in the source language as a job warning. Rodrigo had already ordered `/Lang` when this arrived, so `/Lang` went first. **Assigned** 25 September: Rodrigo said yes in this session, after `/Lang`. **Built** 25 September, v81, branch `feat/verify-widget-text-language`, stacked on #51. It is gate 22, `widget-text`, which always runs and is listed in `GATE_NAMES` after `opt-export-parity`. A tooltip, dropdown display label, or text-field value or default that has a letter in it and reads the same in the output as in the original is REVIEW, listed by full field name. The widget-text mapping marks keeps: `--widget-text PATH`, else the `widget_text.json` beside `--translations`. A deleted key, a slot left out, or a target equal to its source is kept on purpose; a null target is not a decision. The acceptance holds on `corpus/choice_fields.pdf`: stripped without `--widget-text`, it reports the 5 labels by field; translated, or with Color's key deleted in the mapping, it does not. On canary run 4's deliveries: Opus 5.5's 11 strings pass. Opus 5 kept "Kindergarten" on purpose in a separate `widget_text_es.json`, so the default reads the null scaffold beside its mapping and REVIEWs that one label; with `--widget-text` it is silent. `tests/test_widget_text_gate.py` has 9 tests; 8 fail on `eb14a4c`, and the ninth guards the silent case. Evidence: `docs/reviews/2026-09-25-widget-text-gate.md`. **The product checked v81** (`696decb`), in advice for Rodrigo: no effect on the app. The app mentions `GATE_NAMES`, `run_verify` and `VerifyVerdict` only in an import-surface test that checks each with `hasattr`, never by index or count. The diff leaves `strip_text()`, extract and retypeset untouched, so it did not re-run its harness. On its suggestion, `references/gates.md` and `references/widget-text.md` now say to pass `--widget-text` when the authored file is not the one beside `--translations`. Rodrigo approved in this session: it is PR #52, stacked on #51. **done** — v81, PR #52 (merge `0d58202`, 25 September). |

| 2026-09-25 | **Proposal from this library: extract records the source's whole `/Lang`.** `extract_segments` fills segments.json's `document.lang` through PyMuPDF's `doc.language`, which shortens a stored tag: an `en-US` source is recorded as `en`, and a `zh-Hant-TW` one as `zh`. **Acceptance:** a source whose catalog `/Lang` is `en-US` gives `document.lang` `en-US`; verify by a test that stores the tag with the catalog key and reads segments.json. | Found while building the `/Lang` fix, which reads the output's tag whole through `pdf_translate/_lang.py`; the source side was left alone because it is on the app's route. It moves segments.json's bytes for any region-tagged source, so it is a versioned change. | **proposed** 25 September. **Product advice** (25 September, for Rodrigo to schedule): low priority. The product does not object: the app never reads the `document` block. It would re-baseline segments.json at adoption. **Product advice** (26 September, for Rodrigo, who decides): drop for now: no consumer reads it, and it moves segments.json bytes. Reopen it if a consumer asks. |

| 2026-09-26 | **Request from the product: a consumer upgrade note from v54 to v81, and a v81.0.0 tag.** A living document for a Python consumer of `extract_segments`, `strip_text`, `widget_text_scaffold`, `retypeset` and `run_retypeset`. It lists what changes from v54 to v81, the version each change landed in, and whether it moves output bytes; every later version appends to it. Also a `v81.0.0` tag at `0d58202`, for Rodrigo to approve pushing. **Acceptance:** every item on the product's cross-check list appears with its version and whether it moves bytes, and each claim is checked against the code or the version's evidence. The list: `requires-python >=3.14`; the `scale_report.json` envelope (v58); `run_retypeset`, `PlacementError` and `GlyphError` precedence and the refusals shape; the console is not an API; the capture environment variables and `capture_dir`; widget-text keys as fully qualified names, and a cross-version scaffold raising `WidgetTextError` (v62); strip keeping graphics state, which moves bytes (v63); rotated geometry and the stale-extraction refusal (v64); Story escaping of `&`, `<` and `>` (v66); ToUnicode for role faces (v67); extract judging only the pages asked for (v77); `widget_text_kept` (v79); `/Lang` written whole (v80); setuptools >=77 (v73). | The app adopts v81 as one change right after its engine-boundary build lands. The product ruled this on 26 September, and that build is in its final Stage 4 check, so adoption is days away. After adoption, the app's library route for Japanese targets uses extract, strip and legacy retypeset, so a legacy-route defect becomes an app defect. Anything shipped after v81 moves the adoption target, and each consumer-visible change goes in this note. | **requested** 26 September by the app's session. **Product advice:** do first; it is the highest value for adoption. Rodrigo decides. |
