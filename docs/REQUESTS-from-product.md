...
| 2026-09-24 | **Proposal from this library: typography verify checks glyph boxes against the unrotated page.** `typography_verify.py` tests unrotated glyph boxes against the rotated `page.rect`, so a correct typography-1 run near the edge of a `/Rotate 90` page would FAIL. | Found in item 3's coordinate map. It cannot happen today: typography-1 retypeset refuses rotated pages, and group E is held. | **proposed** 24 September; the app noted it. |
