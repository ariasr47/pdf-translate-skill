...
| 2c | Implement kinsoku | The Story engine already does it: 158 lines drawn at six widths, **0** begin with a prohibited character, **0** end with an opener, **0** run past the box; a naive breaker on the same face and widths gives 38 and 8. Per character, **75/75** JLREQ line-start members (cl-02..cl-11, half-width forms included) and **15/15** line-end members (cl-01) are protected. Mechanism: MuPDF `source/html/html-parse.c` classifies with `ucdn_get_resolved_linebreak_class` and a UAX #14 pair table. | **No engine work.** What is missing is a gate that measures the lines actually drawn (the one path that can violate is the author splitting a target across source lines) and a DECISIONS row recording the measurement. Small. |
